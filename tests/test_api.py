...
def server():
    proc = subprocess.Popen(
        [sys.executable, "-m", "uvicorn", "redforge.api.server:app",
         "--host", "127.0.0.1", "--port", "8971", "--log-level", "warning"],
...
def test_health_and_catalog(server):
    h = httpx.get(f"{BASE}/api/health", timeout=10)
    assert h.status_code == 200 and h.json()["ok"] is True
    assert len(httpx.get(f"{BASE}/api/targets", timeout=30).json()) == 11
    assert len(httpx.get(f"{BASE}/api/techniques", timeout=30).json()) == 40


def test_sse_live_campaign_end_to_end(server):
    r = httpx.post(f"{BASE}/api/campaign/start",
    ...
    seen: list[str] = []
    scorecard = None
    with httpx.stream("GET", f"{BASE}/api/events/stream", timeout=300) as s:
    ...
    st = httpx.get(f"{BASE}/api/campaign/status", timeout=30).json()
    ...
    f = httpx.get(f"{BASE}/api/findings", timeout=30).json()
    assert len(f) == st["findings_total"]
    assert all(x["evidence"] for x in f)

    g = httpx.get(f"{BASE}/api/gates", timeout=30).json()
    assert len(g) >= 1
    sig = httpx.post(f"{BASE}/api/gates/{g[0]['id']}/sign?signer=test-lead", timeout=30)
    assert sig.status_code == 200

    b = httpx.get(f"{BASE}/api/briefings", timeout=30).json()
    assert any("verdict" in x["text"].lower() or "complete" in x["text"].lower() for x in b)

    rep = httpx.get(f"{BASE}/api/report", timeout=60).json()
    assert rep["ready"] is True and "RF-F-" in rep["markdown"]

    ev = httpx.get(f"{BASE}/api/events?since=0", timeout=30).json()
...
def test_sse_subscriber_survives_campaign_restart(server):
    ...
    r0 = httpx.post(f"{BASE}/api/campaign/start",
                    json={"packs": ["MEM"], "rounds": 1}, timeout=30)
    assert r0.status_code == 200
    deadline = time.time() + 120
    while time.time() < deadline:
        st0 = httpx.get(f"{BASE}/api/campaign/status", timeout=10).json()
        if not st0["running"]:
            break
        time.sleep(0.5)
    ...
    replay_max = max((e["seq"] for e in
                     httpx.get(f"{BASE}/api/events?since=0", timeout=30).json()), default=0)
    with httpx.stream("GET", f"{BASE}/api/events/stream", timeout=300) as s:
...
def test_abort_and_restart(server):
    a = httpx.post(f"{BASE}/api/campaign/abort", timeout=30).json()
    ...
    while time.time() < deadline:
        if not httpx.get(f"{BASE}/api/campaign/status", timeout=10).json()["running"]:
            break
        time.sleep(0.3)
    st = httpx.get(f"{BASE}/api/campaign/status", timeout=10).json()
    assert st["running"] is False and st["stopped_reason"] == "aborted"
    # restart allowed after abort
    r2 = httpx.post(f"{BASE}/api/campaign/start",
